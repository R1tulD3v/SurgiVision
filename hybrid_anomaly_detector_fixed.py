...
class HybridAnomalyDetector(nn.Module):
    def __init__(self, autoencoder_path):
        ...
        checkpoint = torch.load(autoencoder_path, map_location='cpu', weights_only=True)
...
class AdvancedAnomalyPipeline:
    def __init__(self, hybrid_model_path=None, global_threshold=0.008756):
        ...
        # Load hybrid model
        self.hybrid_model = HybridAnomalyDetector(str(config.AUTOENCODER_PATH))

        if hybrid_model_path and Path(hybrid_model_path).exists():
            checkpoint = torch.load(hybrid_model_path, map_location=self.device, weights_only=True)
            ...
            print("⚠️  Using autoencoder-only mode (hybrid model not trained yet)")
...
def train_hybrid_system():
    ...
    preprocessor = SpleenDataPreprocessor(config.DATA_ROOT)
    ...
    hybrid_model = HybridAnomalyDetector(str(config.AUTOENCODER_PATH))
...
def test_hybrid_system():
    ...
    pipeline = AdvancedAnomalyPipeline(str(config.HYBRID_MODEL_PATH))
...
def main():
    ...
    print("="*60)
    inference.set_global_seed()  # reproducible runs
