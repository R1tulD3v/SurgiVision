...
class Spleen3DAutoencoder(nn.Module):
    ...
    def encode(self, x):
        ...
        # Apply the compression Linear (bottleneck[1]) to get the latent_dim
        # representation. bottleneck[0] is nn.Flatten() and would be a no-op
        # here, returning the uncompressed 32768-d vector instead.
        features = self.bottleneck[1](flattened)
        return features
