...
def test_forward_preserves_shape(model, torch_device):
    x = torch.randn(1, 1, 64, 64, 64, device=torch_device)
    with torch.no_grad():
        y = model(x)
    assert y.shape == (1, 1, 64, 64, 64)


def test_output_in_unit_range(model, torch_device):
...
def test_encode_returns_latent_vector(model, torch_device):
    x = torch.randn(1, 1, 64, 64, 64, device=torch_device)
    with torch.no_grad():
